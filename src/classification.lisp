...
(defclass labeled () ()
  ...
  SOFTMAX-CHUNK. STRIPE-LABEL is implemented as taking the index of
  the prediction with the maximum probability. Thus, only LABEL is
  left to be implemented on the training examples.
...
(defun measure-cross-entropy (examples striped
  ...
  distribution (1 at the label of the class) and the number of
  examples. The length of EXAMPLES must be equal to the number of
  stripes in STRIPED. LABEL-FN takes an example and returns its label.
...
(defun roc-auc (seq pred &key (key #'identity) weight)
  ...
  unnormalized sum within AUC. Else WEIGHT must be a function like
  KEY, but it should return the importance (a positive real number) of
  elements. If the weight of an element is 2 then it's as if there
  were two instances of that element in SEQ.
...
(defun collect-classification-confidences
  ...
  number of examples. The length of EXAMPLES must be equal to the
  number of stripes in STRIPED. LABEL-FN takes an example and returns
  its label that compared by EQL to what STRIPE-LABEL-FN returns for
  STRIPED and the index of the stripe. This is a measurer function."
  (assert (= (length examples) (n-stripes striped)))
...
(defgeneric maybe-make-classification-confidence-collector (obj)
  ...
  args that is invoked when OBJ has the predicted label(s) computed
  and it collects a sequence of confidences (one confidence list per
